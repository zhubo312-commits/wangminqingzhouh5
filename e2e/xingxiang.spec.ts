...
const chart = {
  profile: { name: "测试", gender: "female", genderLabel: "女", yinYangGender: "阴女", solarDateTime: "1990-01-01 12:00", area: "北京市东城", areaCode: "110101", trueSolarTime: null, lunarDate: "一九八九年腊月初五日午时", fiveElementsBureau: "土五局", pillars: { year: "己巳", month: "丙子", day: "丙寅", hour: "甲午" } },
...
test("completes the Xingxiang form, period/year interaction, enlarged chart and restore without overflow", async ({ page }) => {
  ...
  let latestRequest = { name: "测试", gender: "female", birthDateTime: "1990-01-01 12:00", areaCode: "110101", useTrueSolarTime: false, school: "flying" };
  ...
  await page.route("**/api/v1/paipan/xingxiang/context", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ schemaVersion: "guoxue.paipan.xingxiang.v3", chartType: "xingxiang", paipan_ref: paipanRef, generatedAt: "2026-08-12T10:00:00.000Z", expiresAt: "2026-08-13T12:00:00.000Z", chartRequest: latestRequest, chart: { ...chart, profile: { ...chart.profile, name: latestRequest.name, gender: latestRequest.gender, genderLabel: latestRequest.gender === "female" ? "女" : "男", yinYangGender: latestRequest.gender === "female" ? "阴女" : "阴男", solarDateTime: latestRequest.birthDateTime } } }) }));
  ...
  expect(latestRequest).toMatchObject({ name: "测试", gender: "female", areaCode: "110101", useTrueSolarTime: false, school: "flying" });
  await expect(page.getByLabel("运限十二宫星盘")).toHaveCount(0);
  const boardShell = page.locator(".xingxiang-chart-card .xingxiang-board-shell");
  await expect(boardShell).toHaveClass(/is-compact/);
  ...
  expect(await palaceButtons.evaluateAll((buttons) => buttons.filter((button) => button.scrollHeight > button.clientHeight + 1).map((button) => button.getAttribute("aria-label")))).toEqual([]);
  await expect(palaceGrid.locator("button.is-period-life")).toHaveCount(1);
  await expect(palaceGrid.locator("button.is-annual-life")).toHaveCount(0);
  await expect(palaceGrid.locator(".xingxiang-palace-scopes .is-period-change")).toHaveCount(12);
  await expect(palaceGrid.locator(".xingxiang-stars span.is-period-change")).toHaveCount(4);
  await expect(palaceGrid.locator(".xingxiang-stars em.period")).toHaveCount(4);
  expect(await boardShell.evaluate((element) => Math.ceil(element.getBoundingClientRect().height))).toBeLessThanOrEqual(520);
  ...
  await expect(page.locator(".xingxiang-change-summary")).toContainText("未选择流年");
  await expect(page.getByLabel("十二宫变化图例")).toContainText("大限变化流年变化");
  ...
  await expect(page.locator(".xingxiang-change-summary")).toContainText("2005年乙亥流年");
  await expect(palaceGrid.locator("button.is-annual-life")).toHaveCount(1);
  await expect(palaceGrid.locator(".xingxiang-palace-scopes .is-annual-change")).toHaveCount(12);
  await expect(palaceGrid.locator(".xingxiang-stars span.is-annual-change")).toHaveCount(4);
  await expect(palaceGrid.locator(".xingxiang-palace-scopes", { hasText: "正月 · 甲寅" })).toHaveCount(0);
  await expect(palaceGrid.locator(".xingxiang-stars em.period")).toHaveCount(4);
  ...
  await expect(dialog.locator(".xingxiang-board-shell")).not.toHaveClass(/is-compact/);
  await expect(dialog.locator('button[aria-controls="xingxiang-dialog-palace-detail"]')).toHaveCount(12);
  await expect(dialog.locator(".xingxiang-palace-scopes", { hasText: "正月 · 甲寅" })).toHaveCount(1);
